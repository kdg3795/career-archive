...
const projects = [
  ...
  },
  {
    category: "실무 프로젝트",
    period: "2023. 12 — 현재",
    title: "다채움 클래스·멤버 서비스 개발 및 운영",
    overview: "충청북도교육청 교육 플랫폼 다채움에서 클래스 개설부터 구성원 관리, 수업 운영과 학습 콘텐츠 활용까지 지원하는 클래스·멤버 서비스를 개발하고 운영했습니다.",
    responsibilities: [
  ...
  {
    category: "실무 프로젝트 · CSAP 대응",
    period: "2025. 03 — 현재",
    title: "알콩 통합회원 서비스 개발 및 CSAP 보안 대응",
    overview: "한국인터넷진흥원(KISA)의 클라우드 서비스 보안인증(CSAP) 기준에 대응하기 위해 알콩 통합회원 서비스의 기능을 개선하고, 소스코드 보안 취약점을 점검·보완하는 프로젝트입니다.",
    responsibilities: [
      "통합회원 가입·로그인·회원정보 및 약관 기능 개발과 개선",
      "CSAP 소스코드 보안 진단 결과 분석 및 취약점 보완",
      "쿠키 입력값 검증과 응답 헤더 보안 처리",
      "파일 조회·삭제 기능의 경로 조작 방지 로직 적용",
      "세션·로그인·중복 제출 처리의 안정성 개선",
      "파일·Excel 다운로드와 외부 API 예외 처리 개선",
      "보안 조치 이후 기존 기능 영향도 확인 및 테스트",
    ],
    outcomes: [
      "보안 진단 항목별 원인과 영향 범위를 확인하고 기능을 유지하면서 취약 코드를 개선했습니다.",
      "사용자 입력과 파일 경로를 검증해 비정상적인 헤더 삽입과 허용 범위 밖의 파일 접근을 차단했습니다.",
      "세션과 요청 정보가 없는 예외 상황을 방어해 로그인·다운로드 기능의 운영 안정성을 높였습니다.",
      "회원 약관과 개인정보 처리 관련 기능을 보완하고 변경 사항을 테스트 항목과 함께 관리했습니다.",
    ],
    technicalDetails: [
      "쿠키 이름과 값에 포함된 CR·LF 문자를 제거한 뒤 쿠키를 생성하도록 처리하여 HTTP 응답 분할 가능성을 차단했습니다.",
      "레이아웃 소스와 이미지 파일 처리 시 입력 경로를 정규화하고, 설정된 기준 디렉터리 내부의 경로인지 확인한 뒤 조회·삭제하도록 제한했습니다.",
      "요청 컨텍스트와 세션 객체의 존재 여부를 먼저 확인하도록 중복 제출 방지 로직을 개선하고, 공유될 필요가 없는 객체가 Spring 빈으로 관리되지 않도록 잘못된 애너테이션을 제거했습니다.",
      "User-Agent나 요청 헤더가 없는 경우에도 파일·Excel 다운로드와 API 로그 처리가 중단되지 않도록 null 검증과 기본 처리 경로를 추가했습니다.",
      "애플리케이션 기동 시 보안 키 관리 시스템에서 암호화 키를 초기화하고, 키 조회 실패 시 원인을 확인할 수 있는 오류를 남기도록 로그인 암호화 처리를 보완했습니다.",
    ],
    stack: ["Java", "Spring MVC", "eGovFrame", "JSP", "JSTL", "JavaScript", "jQuery", "MyBatis", "MariaDB", "SSE", "REST API", "Tomcat", "Git"],
  },
  {
    category: "실무 프로젝트",
    period: "2023. 12 — 현재",
