...
const coverLetters = [
  {
    ...
    paragraphs: [
      ...
      "백엔드 개발자로 전향한 뒤에는 Java와 Spring 기반의 여러 웹서비스를 개발하고 운영해 왔습니다. 처음에는 요청받은 기능을 정확히 구현하는 데 집중했지만, 실무를 경험할수록 기능 하나도 사용자와 운영 흐름, 기존 시스템에 미치는 영향까지 함께 봐야 한다는 점을 배웠습니다.",
      "신규 기능과 기존 기능 개선은 물론 SQL 작성, 외부 API 연동, 인증과 권한 처리, 파일 처리, 운영 장애 대응까지 경험했습니다. 개발 환경에서는 잘 동작하던 기능이 실제 데이터나 서버 환경에서는 예상과 다르게 움직이는 경우도 있었습니다. 그럴 때마다 로그와 데이터를 비교하고 요청 흐름을 따라가며 원인을 찾았습니다. 덕분에 문제를 막연히 추측하기보다 확인할 수 있는 것부터 하나씩 좁혀가는 습관이 생겼습니다.",
      "프로젝트를 진행하며 기획자, 퍼블리셔, 개발자, 인프라 담당자와 꾸준히 협업했습니다. 같은 요구사항도 역할에 따라 다르게 이해할 수 있기 때문에 제가 파악한 내용을 다시 확인하고, 기술적으로 어려운 부분은 가능한 대안과 함께 설명하려고 했습니다.",
      "요즘은 AI를 코드 분석과 반복 작업에 활용하고 있으며 Docker, Kubernetes, CI/CD도 꾸준히 공부하고 있습니다. 새로운 기술을 많이 아는 것보다 서비스에 필요한 기술을 판단하고 제대로 적용하는 것이 더 중요하다고 생각합니다. 앞으로도 특정 분야에 한정되지 않고 서비스의 흐름을 이해하며, 문제가 생겼을 때 원인을 끝까지 찾아가는 개발자로 일하고 싶습니다.",
    ],
  },
  {
    label: "성격 및 강점",
  ...
  },
  {
    ...
    paragraphs: [
      "웹서비스의 신규 기능 개발부터 기존 시스템 개선, 데이터베이스와 SQL 작업, 외부 API 연동, 배포 이후의 장애 대응까지 경험하며 백엔드 개발자로서 실무 역량을 쌓았습니다. 정해진 기능만 구현하기보다 실제 사용 과정에서 필요한 부분을 살피고, 운영 중 발견된 문제를 개선하는 일도 함께 맡아 왔습니다.",
      "서비스를 운영해 보니 기능 하나는 애플리케이션 코드만으로 완성되지 않았습니다. 데이터베이스와 서버, 인증 체계, 외부 시스템이 서로 연결돼 있었고 한쪽의 작은 변경이 예상하지 못한 곳에 영향을 주기도 했습니다. 장애를 해결하기 위해 로그와 데이터를 따라가다 보니 자연스럽게 개별 기능보다 서비스 전체 구조를 보는 데 관심이 커졌습니다.",
      ...
      "업종이나 서비스가 달라져도 안정적인 기능을 만들고 문제의 원인을 찾아가는 개발의 기본은 같다고 생각합니다. 지금까지 쌓은 경험을 바탕으로 새로운 도메인과 코드도 빠르게 파악하고, 팀원들과 필요한 내용을 정확히 맞추며 오래 운영할 수 있는 서비스를 만들고 싶습니다.",
    ],
  },
  {
    label: "입사 후 포부",
    ...
    paragraphs: [
      ...
      "여러 웹서비스를 개발하고 운영하며 기존 코드를 파악하고 필요한 부분을 개선하는 경험을 쌓았습니다. 새로운 환경에서도 코드와 비즈니스 흐름을 먼저 살피고, 모르는 부분은 확인하면서 맡은 일을 책임 있게 진행하겠습니다.",
